...
class BatchAddWindow(tk.Toplevel):
    def __init__(self, parent, lang_dict):
        ...
        self.ok_button = ttk.Button(btn_frame, text=self.lang_dict['ok_btn'], command=self.on_ok); self.ok_button.pack(side="left", padx=10)
        self.cancel_button = ttk.Button(btn_frame, text=self.lang_dict['cancel_btn'], command=self.destroy); self.cancel_button.pack(side="left")
        self.resizable(True, True); self.text.focus_set()
    def on_ok(self):
        self.result = self.text.get("1.0", "end-1c").strip(); self.destroy()
...
class RuleEditorWindow(tk.Toplevel):
    def __init__(self, parent, lang_dict, rule_data=None, callback=None):
        ...
        self.load_rule_data()
        btn_frame = ttk.Frame(self); btn_frame.grid(row=5, column=0, columnspan=4, pady=10)
    ...
    def load_rule_data(self):
        ...
        date_filter, mode = self.rule_data.get("date_filter", {}), date_filter.get("mode", "none")
    ...
    def save_rule(self):
        ...
        selected_mode = mode_map[self.date_mode.get()]; date_filter = {"mode": selected_mode, "date1": "", "date2": ""}; date1_str, date2_str = self.date1_entry.get().strip(), self.date2_entry.get().strip()
...
class MainApplication(tk.Frame):
    ...
    def setup_ui(self):
        ...
        self.source_listbox = tk.Listbox(self.source_frame, height=4); self.source_listbox.pack(side="left", fill="both", expand=True)
        ...
        bottom_pane = ttk.Frame(self.paned_window); self.paned_window.add(bottom_pane, weight=2)
    ...
    def get_current_settings_data(self):
        return {"sources": list(self.source_listbox.get(0, tk.END)), "recursive": self.recursive_var.get(), "rules": self.rules, "conflict_folder": self.conflict_entry.get()}
    def load_settings_from_data(self, settings):
        ...
        self.conflict_entry.insert(0, settings.get("conflict_folder", "")); self.update_rules_tree()
    ...
    def update_ui_text(self):
        ld = self.lang_dict; self.parent.title(ld['window_title']); self.profile_frame.config(text=ld['profile_frame_title']); self.load_profile_btn.config(text=ld['load_profile_btn']); self.save_profile_btn.config(text=ld['save_profile_btn']); self.save_as_profile_btn.config(text=ld['save_as_profile_btn']); self.delete_profile_btn.config(text=ld['delete_profile_btn']); self.source_frame.config(text=ld['source_frame_title']); self.add_source_btn.config(text=ld['add_source_btn']); self.batch_add_source_btn.config(text=ld['batch_add_source_btn']); self.remove_source_btn.config(text=ld['remove_source_btn']); self.remove_all_sources_btn.config(text=ld['remove_all_sources_btn']); self.recursive_checkbtn.config(text=ld['recursive_checkbtn']); self.rules_frame.config(text=ld['rules_frame_title']); self.rules_tree.heading("enabled", text=ld['col_enabled']); self.rules_tree.heading("name", text=ld['col_name']); self.rules_tree.heading("keyword", text=ld['col_keyword']); self.rules_tree.heading("date_filter", text=ld['col_date_filter']); self.rules_tree.heading("destination", text=ld['col_destination']); self.add_rule_btn.config(text=ld['add_rule_btn']); self.edit_rule_btn.config(text=ld['edit_rule_btn']); self.copy_rule_btn.config(text=ld['copy_rule_btn']); self.delete_rule_btn.config(text=ld['delete_rule_btn']); self.conflict_frame.config(text=ld['conflict_frame_title']); self.browse_conflict_btn.config(text=ld['browse_btn']); self.start_button.config(text=ld['start_button']); self.log_frame.config(text=ld['log_frame_title']); self.lang_button.config(text=ld['lang_button']); self.update_rules_tree()
    ...
    def on_tree_click(self, event):
        ...
        if region == "cell" and self.rules_tree.identify_column(event.x) == '#1':
            ...
            if selected_item_id:
                rule_index = int(self.rules_tree.index(selected_item_id))
    ...
    def start_classification(self):
        sources, conflict_folder, active_rules = list(self.source_listbox.get(0, tk.END)), self.conflict_entry.get(), [r for r in self.rules if r.get("enabled", True)]
        ...
        copied_count, conflicted_count = 0, 0
        for src_path in sources:
            self.log(self.lang_dict['log_scanning'].format(src_path))
            if self.recursive_var.get():
                for root, _, files in os.walk(src_path):
                    for filename in files: file_path = os.path.join(root, filename); c, f = self.process_file(file_path, filename, active_rules, conflict_folder); copied_count += c; conflicted_count += f
            else:
                try:
                    for item in os.listdir(src_path):
                        file_path = os.path.join(src_path, item)
                        if os.path.isfile(file_path): c, f = self.process_file(file_path, item, active_rules, conflict_folder); copied_count += c; conflicted_count += f
                except OSError as e: self.log(self.lang_dict['log_read_error'].format(src_path, e))
        end_time = datetime.now(); self.log(self.lang_dict['log_task_complete'].format(end_time.strftime('%Y-%m-%d %H:%M:%S'))); self.log(self.lang_dict['log_summary'].format(copied_count, conflicted_count)); self.log(self.lang_dict['log_duration'].format((end_time - start_time).total_seconds())); self.start_button.config(state="normal")
    def process_file(self, file_path, filename, active_rules, conflict_folder):
        matching_rules, file_mod_time = [], None
        ...
        for rule in active_rules:
            ...
            date_filter, mode, date_match = rule.get("date_filter", {}), date_filter.get("mode", "none"), False
            if mode == "none": date_match = True
            else:
                try:
                    if mode == "after": d1 = datetime.strptime(date_filter["date1"], "%Y-%m-%d"); date_match = file_mod_time > d1
                    elif mode == "before": d1 = datetime.strptime(date_filter["date1"], "%Y-%m-%d"); date_match = file_mod_time < d1
                    elif mode == "between": d1, d2 = datetime.strptime(date_filter["date1"] + " 00:00:00", "%Y-%m-%d %H:%M:%S"), datetime.strptime(date_filter["date2"] + " 23:59:59", "%Y-%m-%d %H:%M:%S"); date_match = d1 <= file_mod_time <= d2
                except (ValueError, KeyError): date_match = False
            if date_match: matching_rules.append(rule)
        copied_count, conflicted_count = 0, 0
        if len(matching_rules) == 1:
            dest_folder = matching_rules[0]["destination"]
            try:
                if not os.path.exists(dest_folder): os.makedirs(dest_folder)
                shutil.copy2(file_path, os.path.join(dest_folder, filename)); self.log(self.lang_dict['log_copied'].format(filename, matching_rules[0]['name'])); copied_count = 1
            except Exception as e: self.log(self.lang_dict['log_copy_failed'].format(filename, e))
        elif len(matching_rules) > 1:
            try:
                shutil.copy2(file_path, os.path.join(conflict_folder, filename)); rule_names = ", ".join([r['name'] for r in matching_rules]); self.log(self.lang_dict['log_conflict'].format(filename, rule_names)); conflicted_count = 1
            except Exception as e: self.log(self.lang_dict['log_conflict_failed'].format(filename, e))
        return copied_count, conflicted_count
